...
cdef class sgolay2d:
    ...
    def __cinit__(self, np.ndarray[np.uint8_t, ndim=2] z, int window_size, int order ):
       ...
       Z[half_size:-half_size, :half_size] = np.tile( z[:,0].reshape(-1,1), [1,half_size]) - abs( z[:, 1:half_size+1][:,::-1] - np.tile( z[:,0].reshape(-1,1), [1,half_size]) )
       ...
       Z[half_size:-half_size, -half_size:] =  np.tile( z[:,-1].reshape(-1,1), [1,half_size] ) + abs( z[:, -half_size-1:-1][:,::-1] - np.tile( z[:,-1].reshape(-1,1), [1,half_size] ) )
       ...
       Z[:half_size,:half_size] = abs( z[1:half_size+1,1:half_size+1][:,::-1][::-1,...] )
       ...
       Z[-half_size:,:half_size] = Z[-half_size:,half_size].reshape(-1,1) - abs( Z[-half_size:, half_size+1:2*half_size+1][:,::-1] - Z[-half_size:,half_size].reshape(-1,1) )
